...
/// **v4** — Exécute une simulation et retourne, en plus du résultat, les
/// **vues réelles** de chaque agent (état détenu, activité, quiescence).
///
/// Cette fonction existe pour que la couche opérationnelle (v4) puisse
/// construire un état de consensus **mesuré** et non **supposé**. Elle
/// duplique volontairement la boucle de [`simuler`] plutôt que de la
/// modifier : le chemin de code de `simuler` reste strictement inchangé, ce
/// qui préserve la parité bit-à-bit avec la référence Python.
///
/// Retourne `(Resultat, etats, actifs, quiescents)`.
pub fn simuler_avec_vues(
    seed: u64,
    p: &Params,
) -> (Resultat, Vec<Option<Etat>>, Vec<bool>, Vec<bool>) {
    let mut rng = PyRandom::new(seed);
